...
$(function () {
    var submenuCompanies = $('#submenu-companies');
    var active = submenuCompanies.find('.active');
    data = getData(active);
    getCategories(data);

    var submenu = $('#submenu-companies').find('.submenu');
...
function configureLoading() {
    axios.interceptors.request.use(function (config) {
        var subMenu = $("#submenu-categories");
        var row = subMenu.find('.row');
        row.html('');
...
function getData(element) {
    ...
    var company_slug = element.data('company_slug');

    return data = { url: url, company_slug: company_slug };
}

function getCategories(data) {
    axios.get(data.url + '?company_slug=' + data.company_slug).then(function (response) {
        var _response$data = response.data,
            categories = _response$data.categories,
            subcategories = _response$data.subcategories;


        var row = $("#submenu-categories").find('.row');
        row.html('');
...
function createListSubCategory(subcategory) {
    var ulElement = $('#category_' + subcategory.category_id).find('ul');
    var linkElement = $('<a>');
