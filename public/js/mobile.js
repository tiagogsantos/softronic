...
function backMenu(buttonElement) {
    var typeReturn = buttonElement.data('back');
...
function getCategory(element) {
    ...
    axios.get(url + "?company_slug=" + companySlug).then(function (response) {
        ...
        categories.forEach(function (category, index) {
            category.subcategories = subcategories.filter(function (subcategory) {
...
function createMenuCategory(category, companySlug) {
    var submenu = $('#submenu-mobile');
    var hasSubcategory = category.subcategories.length !== 0;
    var clazz = hasSubcategory ? "w-submenu category" : "category";
    var buttonElement = $('<button>', {
        class: clazz,
        text: category.name,
        "data-id": category.id
